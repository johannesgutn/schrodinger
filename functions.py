...
@numba.njit()
def compute_VNc(a11,a12,a21,a22):
    pre=-q/4
    for i1, u1t in enumerate(u1):
        for i2, u2t in enumerate(u2):
            for j1, v1t in enumerate(v1):
...
def compute_nonhom(psi0,t,p1,p2):
    pre = -w/np.pi
    for i1, u1t in enumerate(u1):
        for i2, u2t in enumerate(u2):
            for j1, v1t in enumerate(v1):
                for j2, v2t in enumerate(v2):
                    ...
                    k3_exp = 1j*w*O*(u1t**2+u2t**2)/(2*np.tan(O*t))
...
def fasit1Nc(t,L,p1,p2):
    pre= -2*1j*w/np.cosh(O*t)**2
    
    expo1 = 1j*np.tanh(O*t)/(2*w*O)*(p1**2+p2**2)
    expo2 = -1j*np.tan(O*t)/(2*w*O)*(p1**2+p2**2)
    expo3 = -1j*np.tan(O*L)/(2*w*O)*(p1**2+p2**2)
    
    return pre*np.exp(expo1)*(np.exp(expo2)-np.exp(expo3))

# The expected value of the large-Nc, integrated over time
def fasit1Ncint(L,p1,p2):
    def real_fas(t,L,p1,p2):
        return np.real(fasit1Nc(t,L,p1,p2))
    def imag_fas(t,L,p1,p2):
        return np.imag(fasit1Nc(t,L,p1,p2))
    re = quad(real_fas,0,L,args=(L,p1,p2))[0]
    im = quad(imag_fas,0,L,args=(L,p1,p2))[0]
...
@numba.njit(fastmath = True)
def diff(F,Fother,V,Vother,k,l,i1,i2,j1,j2,h,nonhom):
    ...
    return 1j/(2*w)*(u1der/du1**2+u2der/du2**2-v1der/dv1**2-v2der/dv2**2)+pot+potother-1j*noho

# This solves one time step of the Sch eq through Runge-Kutta4
@numba.njit(fastmath = True, parallel = True)
def compute_psi_runge(psi1,psi2,psi1next,psi2next,k1,k2,k3,l1,l2,l3,nonhom,t,p1,p2):
    h=0
    nonhom = compute_nonhom(nonhom,t+h,p1,p2)
    for i1 in prange(1, Nu1-1):
        for i2 in prange(1, Nu1-1):
            for j1 in prange(1, Nu1-1):
                for j2 in prange(1, Nu1-1):
                    k1[i1,i2,j1,j2]=diff(psi1,psi2,a11,a12,k0,l0,i1,i2,j1,j2,h,nonhom)
                    l1[i1,i2,j1,j2]=diff(psi2,psi1,a22,a21,l0,k0,i1,i2,j1,j2,h,nonhom)
    ...
    for i1 in prange(1, Nu1-1):
        for i2 in prange(1, Nu1-1):
            for j1 in prange(1, Nu1-1):
                for j2 in prange(1, Nu1-1):
                    k2[i1,i2,j1,j2]=diff(psi1,psi2,a11,a12,k1,l1,i1,i2,j1,j2,h,nonhom)
                    l2[i1,i2,j1,j2]=diff(psi2,psi1,a22,a21,l1,k1,i1,i2,j1,j2,h,nonhom)
    ...
    for i1 in prange(1, Nu1-1):
        for i2 in prange(1, Nu1-1):
            for j1 in prange(1, Nu1-1):
                for j2 in prange(1, Nu1-1):
                    k3[i1,i2,j1,j2]=diff(psi1,psi2,a11,a12,k2,l2,i1,i2,j1,j2,h,nonhom)
                    l3[i1,i2,j1,j2]=diff(psi2,psi1,a22,a21,l2,k2,i1,i2,j1,j2,h,nonhom)
    ...
    for i1 in prange(1, Nu1-1):
        for i2 in prange(1, Nu1-1):
            for j1 in prange(1, Nu1-1):
                for j2 in prange(1, Nu1-1):
                    k4=diff(psi1,psi2,a11,a12,k3,l3,i1,i2,j1,j2,h,nonhom)
                    l4=diff(psi2,psi1,a22,a21,l3,k3,i1,i2,j1,j2,h,nonhom)
...
@numba.njit(fastmath = True, parallel = True)
def compute_psi_runge_Nc(psi1,psi2,psi1next,psi2next,k1,k2,k3,l1,l2,l3,nonhom,t,p1,p2):
    h=0
    nonhom = compute_nonhom(nonhom,t+h,p1,p2)
    for i1 in prange(1, Nu1-1):
        for i2 in prange(1, Nu1-1):
            for j1 in prange(1, Nu1-1):
                for j2 in prange(1, Nu1-1):
                    k1[i1,i2,j1,j2]=diff(psi1,psi2,a11Nc,a12Nc,k0,l0,i1,i2,j1,j2,h,nonhom)
                    l1[i1,i2,j1,j2]=diff(psi2,psi1,a22Nc,a21Nc,l0,k0,i1,i2,j1,j2,h,nonhom)
    ...
    for i1 in prange(1, Nu1-1):
        for i2 in prange(1, Nu1-1):
            for j1 in prange(1, Nu1-1):
                for j2 in prange(1, Nu1-1):
                    k2[i1,i2,j1,j2]=diff(psi1,psi2,a11Nc,a12Nc,k1,l1,i1,i2,j1,j2,h,nonhom)
                    l2[i1,i2,j1,j2]=diff(psi2,psi1,a22Nc,a21Nc,l1,k1,i1,i2,j1,j2,h,nonhom)
    ...
    for i1 in prange(1, Nu1-1):
        for i2 in prange(1, Nu1-1):
            for j1 in prange(1, Nu1-1):
                for j2 in prange(1, Nu1-1):
                    k3[i1,i2,j1,j2]=diff(psi1,psi2,a11Nc,a12Nc,k2,l2,i1,i2,j1,j2,h,nonhom)
                    l3[i1,i2,j1,j2]=diff(psi2,psi1,a22Nc,a21Nc,l2,k2,i1,i2,j1,j2,h,nonhom)
    ...
    for i1 in prange(1, Nu1-1):
        for i2 in prange(1, Nu1-1):
            for j1 in prange(1, Nu1-1):
                for j2 in prange(1, Nu1-1):
                    k4=diff(psi1,psi2,a11Nc,a12Nc,k3,l3,i1,i2,j1,j2,h,nonhom)
                    l4=diff(psi2,psi1,a22Nc,a21Nc,l3,k3,i1,i2,j1,j2,h,nonhom)
...
def main(p_GeV,theta):
    ...
    for i in range(1,Nt):
        ...
        psi1,psi2 = compute_psi_runge(psi1,psi2,psi1next,psi2next,k1,k2,k3,l1,l2,l3,psi_nonhom,i*dt,p1,p2)
        
        normals_runge1[i] = integrate(psi1)
        normals_runge2[i] = integrate(psi2)
        
        print(f'time = {i*dt:.3f}: psi1 is {normals_runge1[i]:.3f} (diag Nc is {fasit1Ncint(i*dt,p1,p2):.3f}), and psi2 is {normals_runge2[i]:.3f}')
    ...
    for i in range(1,Nt):
        ...
        psi1Nc,psi2Nc = compute_psi_runge_Nc(psi1Nc,psi2Nc,psi1next,psi2next,k1,k2,k3,l1,l2,l3,psi_nonhom,i*dt,p1,p2)

        normals_runge1Nc[i] = integrate(psi1Nc)
        normals_runge2Nc[i] = integrate(psi2Nc)

        print(f'time = {i*dt:.3f}: psi1 is {normals_runge1Nc[i]:.3f} (diag Nc is {fasit1Ncint(i*dt,p1,p2):.3f}), and psi2 is {normals_runge2Nc[i]:.3f}')

    fasit1Nc_array = np.zeros([Nt]).astype(complex)
    for i, s in enumerate(t):
        fasit1Nc_array[i]=fasit1Ncint(s,p1,p2)
    
    combined = np.vstack((t,normals_runge1,normals_runge2,normals_runge1Nc,normals_runge2Nc,fasit1Nc_array))
    np.save(f'{filename}_p={p_GeV}_theta={theta}_gridpoints={N}_gridsize={ma}_L={tmax}_E={EGev}_z={z}.npy', combined)